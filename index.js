...
// This function moves the rock. (2 pixels at a time seems like a good pace.)
  function moveRock() {
    var rockYString = rock.style.top.replace('px', '')
    var rockY = parseInt(rockYString, 10)
    if (checkCollision(rock) === true) {
      endGame();
    }

    function step() {
      rock.style.top = `${rockY += 2}px`
      // console.log(rock.style.top, rock.style.left);

      if (rockY < GAME_HEIGHT) {
...
function endGame() {
  ...
  for (var junk = 0; junk <= ROCKS.length-1; junk ++) {
    ROCKS[junk].remove();
  }
}
